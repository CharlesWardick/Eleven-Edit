...
function drawTickKnob(canvas, value127, wrap) {
  ...
  // End-stop marks (build 103, Charlie): a short pure-black gap IN the outer
  // ring stroke at the min and max rotation — shows the limits without drawing
  // the eye. (Build 102 put ticks inside the body; wrong place.)
  [0, 127].forEach(function(v) {
    const a = angleFor(v);   // canvas arc angle == the pointer direction
    ctx.beginPath();
    ctx.arc(cx, cy, r, a - 0.07, a + 0.07);
    ctx.strokeStyle = '#202020'; ctx.lineWidth = 2.5; ctx.lineCap = 'butt';   // build 105 (was #1c1c1c b104): fainter (Schrödinger's tick)
    ctx.stroke();
  });
...
function updateBreakupColor() {
  ...
  // build 66: no colour-by-state any more (red-on-change + amber fill removed,
  // matching the pointer knobs); this now only positions the baseline tick.
  ...
  // Position is corrected for the thumb's own width (12px) — the thumb's
...
// STARTUP
// ════════════════════════════════════════════════════════════════════
// Reflect a CMD 0x3D value onto the input dropdown (build 14). The 9 inputs
// (Guitar/Re-Amp/Mic/Line L·R·L+R/Dig L·R·L+R = 0x00..0x08) are the option
// values (as decimal strings). inputVal = -1 / unknown selects the neutral "—"
// placeholder until the rack's connect-time 0x3D query reply lands.
// (Was the GUITAR/MIC/RE-AMP buttons + LINE/DIGITAL pills + read-only clone;
// all replaced by the single dropdown, so this just sets its value.)
function setInputButtons(inputVal) {
  const sel = document.getElementById('chain-input-select');
  if (!sel) return;
  sel.value = (inputVal >= 0x00 && inputVal <= 0x08) ? String(inputVal) : '-1';
}
...
function updateMonoIndicator(isMono) {
  ...
  if (el) {
    // "S | M" button (build 14): the spans are static markup; only the state
    // class changes, which brightens S (stereo) or M (mono) via CSS. Do NOT set
    // textContent here — it would wipe the spans.
